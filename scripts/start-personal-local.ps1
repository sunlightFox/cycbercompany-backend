...
if (-not $NoElevation -and -not (Test-CycberCompanyAdministrator)) {
    $apiTokenHandoffFile = $null
    try {
        $processToken = [Environment]::GetEnvironmentVariable("CYCBERCOMPANY_API_TOKEN", "Process")
...
$nodeConfigDir = Join-Path $env:USERPROFILE ".cycbercompany-node"
...
function Test-CycberCompanyHealth {
...
$script:cycberCompanyApiToken = Read-CycberCompanySecretHandoffFile -Path $ApiTokenFile
...
function Assert-CycberCompanyApiReachable {
    param([string]$ServerUrl)
    $nodesUrl = "$($ServerUrl.TrimEnd('/'))/api/v1/nodes"
    try {
        $response = Invoke-WebRequest -UseBasicParsing -TimeoutSec 3 -Headers (Get-CycberCompanyApiHeaders) -Uri $nodesUrl
        ...
        if ($statusCode -eq 401 -or $statusCode -eq 403) {
            throw "CycberCompany backend is healthy at $ServerUrl, but $nodesUrl rejected the request with HTTP $statusCode. If TOKEN mode is enabled, set CYCBERCOMPANY_API_TOKEN in the current process, User environment, Machine environment, or compose environment."
        }
...
$nodeOnline = {
    param([string]$ServerUrl)
    try {
        $nodes = Invoke-RestMethod -TimeoutSec 3 -Headers (Get-CycberCompanyApiHeaders) `
...
function Save-State {
    ...
    $state = [ordered]@{
        ...
        nodePrivilege = if (Test-CycberCompanyAdministrator) { "Administrator" } else { "Standard user" }
...
function Start-Node {
    ...
    $nodeCommand = "& $(ConvertTo-CycberCompanyPowerShellLiteral $gradle) --no-daemon ':cycbercompany-node-java:run' $(ConvertTo-CycberCompanyPowerShellLiteral $nodeArgs)"
...
function Get-LauncherHealth {
    try {
        ...
        if ($null -eq $response -or $response.service -ne "cycbercompany-local-executor-launcher") {
            return $null
        }
...
function Start-Launcher {
    ...
    $launcherCommand = "& $(ConvertTo-CycberCompanyPowerShellLiteral $launcherScript) " +
        "-Server $(ConvertTo-CycberCompanyPowerShellLiteral $Server) " +
        "-Workspace $(ConvertTo-CycberCompanyPowerShellLiteral $workspacePath) " +
        "-ProjectRoot $(ConvertTo-CycberCompanyPowerShellLiteral $projectRoot) " +
        "-Port $launcherPort"
...
function Get-NodeProcessId {
    ...
    try {
        $match = Get-CimInstance Win32_Process |
            Where-Object {
                $_.CommandLine -like "*cycbercompany-node-java*" -and
...
if (-not $SkipBackend -and -not (Test-CycberCompanyHealth -HealthUrl $healthUrl)) {
    Assert-DockerAvailable
    $backendProcess = Start-Backend
    $startedBackend = $true

    $deadline = [DateTime]::UtcNow.AddSeconds(90)
    while ([DateTime]::UtcNow -lt $deadline -and -not (Test-CycberCompanyHealth -HealthUrl $healthUrl)) {
...
if (-not (Test-CycberCompanyHealth -HealthUrl $healthUrl)) {
    Stop-BackendStack
    if ($SkipBackend) {
...
try {
    Assert-CycberCompanyApiReachable -ServerUrl $Server
} catch {
    Stop-BackendStack
    Clear-State
    throw
...
if (& $nodeOnline $Server) {
    ...
    $needsElevationRestart = (Test-CycberCompanyAdministrator) -and $existingNodePid -and $managedExistingNode `
...
if ($Foreground) {
    try {
        ...
        & .\gradlew.bat --no-daemon ':cycbercompany-node-java:run' "--args=start-local --server $Server --workspace `"$workspacePath`" --config `"$($nodeConfigDir)\local-executor.json`""
    } finally {
        Pop-Location
        Stop-BackendStack
        Stop-Launcher
