...
class NodeRegistrarTest {
    ...
    @Test
    void bootstrapSendsOptionalApiTokenWithoutPersistingItInNodeConfig() throws Exception {
        ...
        try {
            ...
            NodeRegistrar registrar = new NodeRegistrar(
                    new ObjectMapper(),
            ...
            org.assertj.core.api.Assertions.assertThat(authorization).hasValue("Bearer test-token");
    ...
    @Test
    void bootstrapOmitsAuthorizationHeaderWhenApiTokenIsBlank() throws Exception {
        AtomicReference<String> authorization = new AtomicReference<>();
        HttpServer server = HttpServer.create(new InetSocketAddress("127.0.0.1", 0), 0);
        try {
            server.createContext("/api/v1/local-executor/bootstrap", exchange -> {
            ...
            registrar.bootstrapLocalExecutor(
                    "http://127.0.0.1:" + server.getAddress().getPort(),
                    "This computer",
                    SystemInfo.current());
