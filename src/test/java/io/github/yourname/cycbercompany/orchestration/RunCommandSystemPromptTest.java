...
class RunCommandSystemPromptTest {
    ...
    void codingRunsReceiveWorkspaceScopeAndVerificationWorkflow() {
        ...
        assertThat(prompt)
                ...
                .contains("no write is allowed anywhere, including temporary locations")
                .contains("do not run Gradle or Maven merely to identify a project")
                .contains("never use `tasks --all` during a general inspection")
                .contains("Do not append `|| echo`")
                ...
                .contains("explicitly cover every requested outcome")
    ...
    void inProcessLocalRunsTellTheAgentThatToolsOperateOnTheUserMachine() {
        ...
        assertThat(prompt)
                ...
                .contains("including /tmp and other")
                .contains("temporary locations. Keep shell observations")
                .contains("do not use >, >>, tee, touch")
                .contains("Never use system.shell.run, sudo, or raw systemctl")
                .contains("system.service.restart only when that exact capability is advertised")
                .contains("converts a failed command into a successful exit status")
                .contains("curl -ksS --fail --max-time 5")
                .contains("Do not redirect output, write a")
                .contains("temporary file, or broaden the URL.");
    }
    ...
    void explicitLocalProjectPathGetsTheSameControlledDevelopmentToolSet() {
        assertThat(RunCommandService.requestsLocalProject(
                "Please fix the backend project at D:\\ai\\spring-cycbercompany-backend and run its tests."))
                .isTrue();
        assertThat(RunCommandService.requestsLocalProject(
        ...
        assertThat(RunCommandService.requestsLocalProject(
                ...
                .isTrue();
        assertThat(RunCommandService.requestsLocalProject(
                "Explain how to fix the project at D:\\ai\\spring-cycbercompany-backend."))
                .isFalse();
        assertThat(RunCommandService.requestsLocalProject(
    ...
    @Test
    void currentWorkspaceDoesNotTriggerAnUnrelatedWebSearch() {
