...
class ConversationRunQueueTest {
    ...
    @Test
    void cancellingTheActiveRunReleasesTheSlotAndStartsTheNextRun() {
        ConversationRunQueue queue = new ConversationRunQueue();
        List<String> started = new ArrayList<>();
        ...
        queue.cancel(QUEUE, "run-1");

        assertThat(started).containsExactly("run-1", "run-2");
        assertThat(queue.position(QUEUE, "run-1")).isNull();
        assertThat(queue.position(QUEUE, "run-2")).isZero();
    }
}
