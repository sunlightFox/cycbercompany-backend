...
public class ToolApprovalService {
    ...
    /** Expires stale requests so their waiting Runs can be finalized by the coordinator. */
    @Transactional
    public List<ExpiredToolApproval> expireOutstanding() {
        Instant now = Instant.now();
        return repository.findByStatusAndExpiresAtLessThanEqual(ToolApprovalStatus.REQUESTED, now).stream()
                .filter(approval -> approval.expireIfPending(now))
                .map(approval -> {
                    repository.save(approval);
                    return new ExpiredToolApproval(approval.id(), approval.tenantId(), approval.runId());
                })
                .toList();
    }
    ...
    public record ExpiredToolApproval(String approvalId, String tenantId, String runId) {
    }
}
