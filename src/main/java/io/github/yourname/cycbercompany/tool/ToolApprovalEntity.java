...
class ToolApprovalEntity {
    ...
    boolean expireIfPending(Instant now) {
        if (status != ToolApprovalStatus.REQUESTED || expiresAt == null || expiresAt.isAfter(now)) {
            return false;
        }
        status = ToolApprovalStatus.EXPIRED;
        decidedAt = now;
        errorMessage = "Tool approval expired before a decision was received.";
        return true;
    }
