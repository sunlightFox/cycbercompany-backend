...
import io.github.yourname.cycbercompany.agent.AgentRuntimeDefinitionService;
import io.github.yourname.cycbercompany.config.AppProperties;
import io.github.yourname.cycbercompany.conversation.ConversationAttachmentService;
import io.github.yourname.cycbercompany.conversation.ConversationService;
import io.github.yourname.cycbercompany.conversation.MessageRole;
...
import io.github.yourname.cycbercompany.memory.MemoryRetrievalService;
import io.github.yourname.cycbercompany.memory.MemorySnapshot;
import io.github.yourname.cycbercompany.memory.MemoryCandidateService;
import io.github.yourname.cycbercompany.persona.UserPersonaContext;
...
import io.github.yourname.cycbercompany.mcp.McpToolCallResult;
import io.github.yourname.cycbercompany.node.NodeToolApprovalDecisionView;
import io.github.yourname.cycbercompany.node.NodeToolApprovalStatus;
...
import io.github.yourname.cycbercompany.skill.SkillAnalysis;
...
import io.github.yourname.cycbercompany.tool.WebSearchTrace;
...
import io.github.yourname.cycbercompany.tool.ToolApprovalDecisionView;
...
public class RunCommandService {
    ...
    private static final String INTERNAL_REASONING_TAG = "(?:mm:)?(?:think|thinking|analysis|reasoning)";
    ...
    public RunView cancel(String runId, ActorContext actor) {
        ...
        scheduleAfterCommit(() -> queue.cancel(queueKey, runId));
        return RunView.from(run);
    }

    /** Finalizes a Run whose pending tool approval elapsed without a reviewer decision. */
    @Transactional
    public void failExpiredToolApproval(String runId, String tenantId, String approvalId) {
        runs.findByIdAndTenantId(runId, tenantId).ifPresent(run -> {
            if (run.status() != RunStatus.WAITING_APPROVAL) {
                return;
            }
            ActorContext actor = new ActorContext(run.tenantId(), run.userId(), Set.of(), Set.of());
            String answer = "Tool approval expired before a decision was received. The requested operation was not executed.";
            continuations.findByRunIdAndTenantId(runId, tenantId).ifPresent(continuations::delete);
            run.fail("Tool approval expired: " + approvalId, answer);
            runs.saveAndFlush(run);
            if (executionTasks != null) {
                executionTasks.completeFromRun(runId, run.status());
            }
            if (workflowCheckpoints != null) {
                workflowCheckpoints.failure(runId, actor, answer);
            }
            conversations.append(run.conversationId(), MessageRole.ASSISTANT, answer, runId, actor);
            events.publish(runId, RunEventType.RUN_FAILED, answer, actor);
            var queueKey = new ConversationRunQueue.QueueKey(tenantId, run.conversationId());
            scheduleAfterCommit(() -> queue.complete(queueKey, runId));
        });
    }
    ...
    private void executeQueued(String runId) {
        ...
        try {
            log.info("Starting queued worker body for run {}.", runId);
            persisted = runs.findById(runId)
                    .orElseThrow(() -> new IllegalArgumentException("Run not found: " + runId));
            log.info("Loaded queued run {} with status {}.", runId, persisted.status());
            RunSpec spec = deserializeRunSpec(persisted);
            log.info("Deserialized RunSpec for run {} with mode {}.", runId, spec.executionMode());
            actor = spec.actor();
            queueKey = new ConversationRunQueue.QueueKey(actor.tenantId(), spec.conversationId());
            if (!claimPersistentTask(runId)) {
                log.info("Skipped queued run {} because persistent task was already claimed.", runId);
                releaseQueueSlotWhenTerminal(runId, actor, queueKey);
                return;
            }
            log.info("Claimed persistent task for run {}.", runId);
    ...
    private void execute(String runId, RunSpec spec, ActorContext actor) {
        ...
        try {
            log.info("Entering execution body for run {}.", runId);
            CodingWorkspaceScope workspaceScope = CodingWorkspaceScope.from(command.workingDirectory());
            log.info("Resolved workspace scope for run {} as {}.", runId, workspaceScope.relativePath());
            AgentRunEntity run = runs.findById(runId)
                    .filter(candidate -> actor.tenantId().equals(candidate.tenantId()))
                    .orElseThrow(() -> new IllegalArgumentException("Run not found: " + runId));
            log.info("Loaded executable run {} with status {}.", runId, run.status());
            if (run.status() != RunStatus.QUEUED && run.status() != RunStatus.CREATED) {
                return;
            }
            run.start();
            // Persist the terminal state before publishing FINAL_ANSWER. Event publication
            // acquires its own pessimistic run lock; a plain deferred save can otherwise
            // leave the durable Run in RUNNING after clients have received a final answer.
            runs.saveAndFlush(run);
            log.info("Persisted RUNNING status for run {}.", runId);
            if (workflowCheckpoints != null) {
                log.info("Updating inspecting workflow checkpoint for run {}.", runId);
                workflowCheckpoints.phase(runId, actor, RunWorkflowPhase.INSPECTING);
                log.info("Updated inspecting workflow checkpoint for run {}.", runId);
            }
            log.info("Publishing RUN_STARTED event for run {}.", runId);
            events.publish(runId, RunEventType.RUN_STARTED, "Run accepted by local coordinator.", actor);
            log.info("Published RUN_STARTED event for run {}.", runId);
    ...
    private void executeResumedCoding(
            ...
            ActorContext actor) {
        try {
            log.info("Entering resumed execution body for run {}.", runId);
            AgentRunEntity run = runs.findById(runId)
                    .filter(candidate -> actor.tenantId().equals(candidate.tenantId()))
                    .orElseThrow();
            log.info("Loaded resumed run {} with status {}.", runId, run.status());
            RunSpec spec = deserializeRunSpec(run);
    ...
    private ModelGateway.ModelAnswer completeModelCall(
            ...
            ActorContext actor) {
        return modelCallWithRetry(runId, phase, request, actor, () -> modelGateway.complete(request));
    }

    private ModelGateway.ModelAnswer streamModelCall(
            String runId,
            String phase,
            ModelGateway.ModelCompletionRequest request,
            ActorContext actor) {
    ...
    private ModelGateway.ModelAnswer modelCallWithRetry(
        ...
        for (int retry = 0; ; retry++) {
            try {
                long startedNanos = System.nanoTime();
                ModelGateway.ModelAnswer answer = call.execute();
                RunModelUsage.publish(
                        events, objectMapper, runId, phase, request.modelProfileId(), answer, startedNanos, actor);
                return answer;
            } catch (ModelRateLimitException ex) {
                if (retry >= MAX_MODEL_RECOVERY_RETRIES) {
                    throw new IllegalStateException("Model provider continued rate limiting after "
                            + MAX_MODEL_RECOVERY_RETRIES + " retries.", ex);
                }
                Duration delay = retryDelay(ex.retryAfter(), retry);
                events.publish(runId, RunEventType.MODEL_RATE_LIMITED,
                        "retry=" + (retry + 1) + ", delaySeconds=" + delay.toSeconds(), actor);
                waitForModelRecovery(delay);
            } catch (ModelTransientException ex) {
                if (retry >= MAX_MODEL_RECOVERY_RETRIES) {
                    throw new IllegalStateException("Model provider continued failing transiently after "
                            + MAX_MODEL_RECOVERY_RETRIES + " retries.", ex);
                }
                Duration delay = retryDelay(null, retry);
                String status = ex.statusCode() == null ? "transport" : ex.statusCode().toString();
                events.publish(runId, RunEventType.MODEL_PROVIDER_RETRYING,
                        "retry=" + (retry + 1) + ", status=" + status
                                + ", delaySeconds=" + delay.toSeconds(), actor);
                waitForModelRecovery(delay);
            }
        }
    }

    private static Duration retryDelay(Duration providerDelay, int retry) {
        if (providerDelay != null && !providerDelay.isNegative() && !providerDelay.isZero()) {
            return providerDelay.compareTo(MAX_MODEL_RECOVERY_DELAY) > 0
                    ? MAX_MODEL_RECOVERY_DELAY : providerDelay;
        }
        Duration delay = INITIAL_MODEL_RECOVERY_DELAY.multipliedBy(1L << Math.min(retry, 3));
        return delay.compareTo(MAX_MODEL_RECOVERY_DELAY) > 0 ? MAX_MODEL_RECOVERY_DELAY : delay;
    }

    private static void waitForModelRecovery(Duration delay) {
        try {
            Thread.sleep(delay);
        } catch (InterruptedException ex) {
            Thread.currentThread().interrupt();
            throw new IllegalStateException("Run was interrupted while waiting for model provider recovery.", ex);
        }
    }

    @FunctionalInterface
    private interface ModelCall {
        ModelGateway.ModelAnswer execute();
    }
    ...
    private void failUnlessCancelled(String runId, Exception ex, ActorContext actor) {
        String failureMessage = userVisibleFailureMessage(runId, ex, actor);
        boolean cancelled = runs.findByIdAndTenantId(runId, actor.tenantId()).map(run -> {
            if (run.status() == RunStatus.CANCELLED) {
                return true;
            }
            run.fail(safeFailureMessage(ex), failureMessage);
            runs.save(run);
            // A failed run must still leave a visible assistant message.  The UI can lose its
            // live SSE connection, and error-only Run records otherwise render as an empty reply.
            conversations.append(run.conversationId(), MessageRole.ASSISTANT, failureMessage, runId, actor);
            if (workflowCheckpoints != null) {
                workflowCheckpoints.failure(runId, actor, failureMessage);
            }
            return false;
        }).orElse(false);
        if (!cancelled) {
            events.publish(runId, RunEventType.RUN_FAILED, failureMessage, actor);
        }
    }

    private String userVisibleFailureMessage(String runId, Exception failure, ActorContext actor) {
        List<String> completed = events.replay(runId, 0L, actor).stream()
                .filter(event -> event.type() == RunEventType.STEP_COMPLETED
                        || event.type() == RunEventType.TOOL_CALL_COMPLETED)
        ...
        StringBuilder message = new StringBuilder("本次任务未能完整完成，但执行过程不会静默结束。\n\n");
        if (completed.isEmpty()) {
            message.append("已完成：已完成任务准备和执行尝试。\n");
        } else {
            message.append("已完成：\n");
        ...
        message.append("\n当前受阻：").append(friendlyFailureDetail(failure)).append('\n');
        message.append("\n后续建议：请根据上述受阻原因调整后重试；已完成的步骤和实际工具执行记录仍可在本次运行详情中查看。");
        return message.toString();
    }

    private static String friendlyFailureDetail(Exception failure) {
        String detail = safeFailureMessage(failure);
        String normalized = detail.toLowerCase(Locale.ROOT);
        if (normalized.contains("local http service did not become ready")
                || normalized.contains("invocation timed out and node status could not be confirmed")) {
            return "本地服务在限定时间内未能确认就绪，因此无法完成启动验证。";
        }
        if (normalized.contains("loopback") || normalized.contains("private host")) {
            return "浏览器安全策略禁止访问本机或私网地址，因此无法完成浏览器验证。";
        }
        return detail;
    }
    ...
    public void recoverPersistedRun(String runId) {
        ...
        int queuePosition = queue.reserve(
                queueKey, run.id(), () -> executions.submit(run.id(), () -> executeQueued(run.id())));
        log.info("Recovered run {} into conversation queue at position {}.", run.id(), queuePosition);
        scheduleAfterCommit(() -> {
            log.info("Activating recovered run {} conversation queue.", run.id());
    ...
    private boolean claimPersistentTask(String runId) {
        ...
        boolean claimed = executionTasks.claim(runId, "runlease_" + UUID.randomUUID()).isPresent();
        log.info("Persistent task claim for run {} returned {}.", runId, claimed);
        return claimed;
    }
    ...
    private static void appendNodeInteractionWorkflow(StringBuilder builder, CreateRunCommand command) {
        builder.append("""
                ...
                - When the user requests a read-only operation or says not to modify files, do not create,
                  rewrite, delete, or redirect output to any file anywhere on the host, including /tmp and other
                  temporary locations. Keep shell observations on stdout/stderr: do not use >, >>, tee, touch,
                  or a write-then-delete workaround.
                ...
                - For system.shell.run, only the command is required. Omit cwd unless the user's current request
                  ...
                  configured default.
                - Never use system.shell.run, sudo, or raw systemctl to imitate a missing structured service action.
                  For a restart, use system.service.restart only when that exact capability is advertised; otherwise
                  state that the selected node cannot safely perform the restart. Never append echo, printf, `true`,
                  or another construct that converts a failed command into a successful exit status.
                ...
                - When validating a local HTTP or HTTPS endpoint and browser access rejects its loopback or private
                  address, use the advertised system.shell.run capability for one short-lived stdout-only probe instead:
                  curl -ksS --fail --max-time 5 with the exact user-requested URL. Do not redirect output, write a
                  temporary file, or broaden the URL. State the certificate-bypass limitation when -k is required.
    ...
    private static void appendCodingWorkflow(StringBuilder builder, CodingWorkspaceScope workspaceScope) {
        builder.append("""
                ...
                - When the user requests inspection only or says not to modify files, no write is allowed anywhere, including temporary locations. Keep shell output on stdout/stderr and do not use >, >>, tee, touch, or a write-then-delete workaround.
                ...
                - Do not pipe Gradle or Maven commands through head, tail, grep, or similar output consumers: the shell runner already bounds captured output and Java build processes can remain alive behind a closed pipeline. Run the manifest-backed command directly. A project inspection already proves the build system and manifests, so do not run Gradle or Maven merely to identify a project. Run a build tool only when the user explicitly requests a build, test, version, dependency, or task-list result. For a requested Gradle task list, first use `./gradlew --offline --no-daemon --console=plain tasks` with a short timeout; never use `tasks --all` during a general inspection and never trigger a distribution or dependency download solely to inspect. Do not append `|| echo` or another construct that hides a failed build-tool exit status. For Gradle checks, retain --no-daemon --console=plain when the inspected recommendation contains them. After one command timeout, inspect its result and choose a strictly narrower, different check; never retry the same build family with a longer timeout.
                ...
                - In the final answer, explicitly cover every requested outcome, including each requested inspection result even when no files changed. State the files changed, the concrete verification performed, any process URL that remains running, and any limitation that was not verified.
                """);
    ...
    static boolean requestsExternalSearch(String text) {
        ...
        return normalized.contains("\u8054\u7f51")
                ...
                || normalized.matches("(?s).*\\bcurrent\\s+(?:news|events|information|weather|price|prices|status)\\b.*")
    ...
    private CreateRunCommand resolveExecutionTarget(
        ...
        // A personal-local executor was historically represented by a persisted local_* node id.
        // Keep existing browser sessions working after a database migration by routing that legacy
        // marker to the backend-owned executor, which does not need a node record or WebSocket.
        boolean legacyManagedLocalExecutor = requestedNodeId != null
                && requestedNodeId.startsWith("local_")
                && !nodes.isManagedLocalNode(requestedNodeId, actor);
        ...
        if (legacyManagedLocalExecutor) {
            resolvedNodeId = InProcessLocalToolProvider.TARGET_ID;
        } else if (automaticNode && !systemOperationRequested) {
    ...
    private String replayVisibleTokenDeltas(String runId, ActorContext actor) {
        return events.replay(runId, 0L, actor).stream()
                .filter(event -> event.type() == RunEventType.TOKEN_DELTA)
                .map(RunEventView::payload)
