...
public class RunRecoveryCoordinator {
    ...
    public void recoverAtStartup() {
        recoverReadyTasks();
        // A JVM restart terminates every local worker. Do not wait for the old
        // lease timeout before making those runs visible and retryable.
        for (RunExecutionTaskEntity task : tasks.findRunningTasks()) {
            runs.markRunRecoveryUnknown(task.runId(),
                    "The control plane restarted before the Run outcome could be safely recovered.");
        }
    }
