...
public interface RunExecutionOutboxRepository extends JpaRepository<RunExecutionOutboxEntity, String> {
    ...
    @Query("""
            ...
            where (message.status = io.github.yourname.cycbercompany.orchestration.RunExecutionOutboxStatus.PENDING
                or (message.status = io.github.yourname.cycbercompany.orchestration.RunExecutionOutboxStatus.DISPATCHING
