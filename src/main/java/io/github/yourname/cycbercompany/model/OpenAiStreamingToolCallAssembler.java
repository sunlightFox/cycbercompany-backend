...
final class OpenAiStreamingToolCallAssembler {
    ...
    void accept(JsonNode toolCalls) {
        ...
        for (int position = 0; position < toolCalls.size(); position++) {
            ...
            appendFragment(call.id, fragment.get("id"));
            ...
            if (function != null && !function.isNull()) {
                ...
                appendFragment(call.name, function.get("name"));
                appendFragment(call.arguments, function.get("arguments"));
            }
        }
    }
    ...
    /**
     * Providers disagree on whether a streamed field is a delta or the complete
     * value seen so far. Accept both forms without duplicating repeated fields.
     */
    private static void appendFragment(StringBuilder target, JsonNode value) {
        if (value == null || value.isNull()) {
            return;
        }
        String fragment = value.asText();
        if (fragment.isEmpty()) {
            return;
        }
        ...
        } else if (fragment.equals(current)) {
            // Repeated complete value.
            return;
        } else if (fragment.startsWith(current)) {
            // Cumulative value: replace the shorter prefix with the latest value.
            target.setLength(0);
            target.append(fragment);
        } else {
            target.append(fragment);
