...
import io.github.yourname.cycbercompany.nodeclient.config.NodeConfigStore;
import io.github.yourname.cycbercompany.nodeclient.config.NodeProcessLock;
import io.github.yourname.cycbercompany.nodeclient.protocol.NodeRegistrar;
import io.github.yourname.cycbercompany.nodeclient.transport.NodeWebSocketClient;
 ...
 */
public class CycberCompanyNodeApplication {

    public static void main(String[] args) throws Exception {
        // Keep TLS encrypted while omitting SNI for networks that reset SNI handshakes.
    ...
    static void register(
        ...
        NodeConfig registered = registrar.register(server, name, SystemInfo.current());
        Path workspace = workspacePath(options);
        // The packaged client is intended to answer explicit desktop/system questions.
        // Keep server-side approval gates in force, but register with system capability
        // discovery enabled so read-only desktop tools are available after first launch.
    ...
    private static void autoStart(
        ...
        if (!java.nio.file.Files.exists(configStore.path())) {
            String server = firstNonBlank(options.get("server"), System.getenv("CYCBERCOMPANY_SERVER"));
            if (server == null) {
                throw new IllegalArgumentException(
                        "First launch requires --server or CYCBERCOMPANY_SERVER.");
            }
            register(optionsWith(options, "server", server), configStore, objectMapper, httpClient);
        }
        start(options, configStore, objectMapper, httpClient);
    }
    ...
    private static Map<String, String> optionsWith(Map<String, String> options, String key, String value) {
    ...
    private static Path defaultConfigPath() {
        return Path.of(System.getProperty("user.home"), ".cycbercompany-node", "config.json");
    }
    ...
    private static void printUsage() {
        System.out.println("""
                ...
                Commands:
                  register --server http://localhost:8080 [--name my-pc] [--workspace path] [--access workspace|system] [--config node-config.json]
                  ...
                  auto [--server URL] [--workspace path] [--config node-config.json]
    ...
    static Path localConfigPath(Map<String, String> options) {
        ...
        return Path.of(System.getProperty("user.home"), ".cycbercompany-node", "local-executor.json");
    }
