...
```powershell
..\gradlew.bat :cycbercompany-node-java:run --args="register --server http://localhost:8080 --name my-pc"
```

## 启动节点

```powershell
..\gradlew.bat :cycbercompany-node-java:run --args="start"
```
...
```powershell
..\gradlew.bat :cycbercompany-node-java:run --args="register --server http://localhost:8080 --name my-pc --workspace D:\ai\project --access system"
..\gradlew.bat :cycbercompany-node-java:run --args="start"
```
...
```powershell
..\gradlew.bat :cycbercompany-node-java:run --args="register --server http://localhost:8080 --name my-pc --config .\node-config.json"
..\gradlew.bat :cycbercompany-node-java:run --args="start --config .\node-config.json"
```
